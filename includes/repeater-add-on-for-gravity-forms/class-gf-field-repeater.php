...
class GF_Field_Repeater extends GF_Field
{
    ...
    public static function gform_hide_children($form)
    {
        ...
        foreach ($form['fields'] as $key => $field) {
            ...
            } elseif ($field->type == 'repeater-end') {
                array_push($repeaterChildren, $field->id);
            }
        ...
        if ($grid_modified) {
            GFFormsModel::update_grid_column_meta($form_id, $grid_meta);
        }
                        ...
                        if (in_array($getInputIdNum[1], $repeater_required)) {
                            ...
                            switch ($fieldType) {
                                case 'name':
                                    ...
                                    if (in_array($getInputIdNum[2], $requiredIDs) && empty($getInputData)) {
                                        $failedValidation = true;
                                    }
                                    break;
                                case 'address':
                                    $skipIDs = array(2);
                                    if (!in_array($getInputIdNum[2], $skipIDs) && empty($getInputData)) {
                                        $failedValidation = true;
                                    }
                                    break;
                                default:
    ...
    public function get_field_content($value, $force_frontend_label, $form)
    {
        if (is_admin()) {
            ...
            if (!empty($field_label)) {
                $field_label = "<h2 class='gf_repeater_title'>{$field_label}</h2>";
            } else {
                $field_label = '';
            }
            if ($hide_label) {
