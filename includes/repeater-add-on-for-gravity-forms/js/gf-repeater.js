...
function gfRepeater_getRepeaters() {
    var repeaterData = jQuery('.gform_wrapper').each(function () {
        ...
        if (jQuery(form).attr('action') == '/ajax-test') {
        ...
        jQuery(this).find('.gfield').each(function () {
            if (repeaterFound == 0) {
                if (jQuery(this).has('.ginput_container_repeater').length) {
                    ...
                    if (gfRepeater_debug) {
                        console.log('Form #' + formId + ' - Repeater #' + repeaterId + ' - Start: ' + jQuery(this).attr('id'));
                    }
                    ...
                    if (repeaterInfo) {
                        repeaterInfo = JSON.parse(repeaterInfo);
                    }
                ...
                if (jQuery(this).has('.ginput_container_repeater-end').length) {
                    ...
                    if (!repeaterStart || (repeaterMax && repeaterStart > repeaterMax)) {
                        repeaterStart = 1;
                    }
                    if (!repeaterMin || (repeaterMax && repeaterMin > repeaterMax)) {
                        repeaterMin = 1;
                    }
                    if (!repeaterMax || (repeaterMin && repeaterMax && repeaterMin > repeaterMax)) {
                        repeaterMax = null;
                    }
                    ...
                    if (childInfo['inputMask']) {
                        inputMask = childInfo['inputMask'];
                    }
                    ...
                    if (gfRepeater_debug) {
                        console.log('Form #' + formId + ' - Repeater #' + repeaterId + ' - Child #' + repeaterChildCount + ' - Found: ' + childId);
                    }
                    ...
                    jQuery(this).find(':input').each(function () {
                        ...
                        if (inputName) {
                            if (jQuery.inArray(inputName, childInputNames) == -1) {
                                childInputNames.push(inputName);
                            }
                            if (inputName.slice(-2) == '[]') {
                                inputName2 = inputName.slice(0, inputName.length - 2);
                            } else {
                            ...
                            if (childInfo['prePopulate']) {
                                ...
                                if (inputPrePopulate) {
                                    jQuery.each(inputPrePopulate, function (key, value) {
                                        if (key > repeaterParemCount) {
                                            repeaterParemCount = Number(key);
                                        }
                                    });
                                }
                            }
                        ...
                        if (gfRepeater_debug) {
                            console.log('Form #' + formId + ' - Repeater #' + repeaterId + ' - Child #' + repeaterChildCount + ' - Input Found: ' + inputId);
                        }
                    });
        ...
        if (gfRepeater_debug) {
            console.log('Form #' + formId + ' - Repeaters Found: ' + (repeaterId));
        }
        if (repeaterFound !== 0) {
            return false;
        }
...
function gfRepeater_setRepeaterChildAttrs(formId, repeaterId, repeaterChildElement, repeatId) {
    ...
    if (childKey) {
        ...
        jQuery.each(repeater['children'][childKey]['inputs'], function (key, value) {
            ...
            if (inputMask) {
                jQuery(inputElement).mask(inputMask);
            }
            ...
            if (gfRepeater_submitted && checkValidation) {
                ...
                if (childRequired) {
                    if (newInputName) {
                        ...
                        if (childType == 'name' && jQuery.inArray(splitName[4], ['3', '6']) == -1) {
                            return true;
                        }
                        if (childType == 'address' && jQuery.inArray(splitName[4], ['2']) !== -1) {
                            return true;
                        }
                    }
...
function gfRepeater_conditionalLogic_do(formId, repeaterId, repeaterChildId, repeatId) {
    ...
    jQuery.each(conditionalLogic['rules'], function (key, value) {
        ...
        if (multiInput) {
            if (jQuery.inArray(value['value'], inputValue) !== -1) {
    ...
    if (conditionalLogic['logicType'] == 'all') {
        if (jQuery.inArray(false, conditions) == -1) {
            conditionsPassed = true;
        }
    } else {
        if (jQuery.inArray(true, conditions) !== -1) {
            conditionsPassed = true;
        }
    }
    ...
    if (repeaterChild['type'] == 'section') {
        ...
        if (sectionChildren !== false) {
            effectedIds = effectedIds.concat(sectionChildren);
        }
    }
...
function gfRepeater_repeatRepeater(formId, repeaterId) {
    var repeater = gfRepeater_repeaters[formId][repeaterId];
    var repeatId = repeater['data']['repeatCount'] + 1;
    if (repeater['settings']['max'] && repeater['data']['repeatCount'] >= repeater['settings']['max']) {
        return;
    }
    ...
    var elemWillAppendTo = jQuery('.gf_repeater_add').parents('li');
    ...
    jQuery.each(repeater['children'], function (key, value) {
        ...
        clonedElement.insertBefore(elemWillAppendTo);
    ...
    if (jQuery("[ccb-field='event.register_user_type']").length > 0) {
        if ('family' == jQuery("[ccb-field='event.register_user_type']").val()) {
            if (typeof user_profile_data != 'undefined') {
                if (jQuery('[ccb-field="individual.family.id"]').length > 0) {
                    jQuery('[ccb-field="individual.family.id"]').val(user_profile_data['individual.family.id']);
                }
            }
        }

        if (repeater['data']['repeatCount'] <= 2) {
            jQuery('#gform_' + formId).find('.gf_repeater_remove').hide();
        } else {
            jQuery('#gform_' + formId).find('.gf_repeater_remove').show();
        }
    }

    if (gfRepeater_debug) {
        console.log('Form #' + formId + ' - Repeater #' + repeaterId + ' - repeated');
    }
}
...
function gfRepeater_unrepeatRepeater(formId, repeaterId, repeatId) {
    ...
    if (!repeatId) {
        var repeatId = repeater['data']['repeatCount'];
    }
    ...
    if (gfRepeater_debug) {
        console.log('Form #' + formId + ' - Repeater #' + repeaterId + ' - Repeat #' + repeatId + ' - unrepeated');
    }
}
...
function gfRepeater_select(formId, repeaterId, repeatId, childId, inputId) {
    ...
    if (repeaterId || repeatId || childId || inputId) {
        selector += '>.gform_body .gform_fields>.gfield.gf_repeater_child_field';
    }
    if (repeaterId) {
        selector += '[data-repeater-parentid=' + repeaterId + ']';
    }
    if (repeatId) {
        selector += '[data-repeater-repeatid=' + repeatId + ']';
    }
    if (childId) {
        selector += '[data-repeater-childid=' + childId + ']';
    }
    if (inputId) {
        if (inputId == '*') {
...
function gfRepeater_start() {
    jQuery.each(gfRepeater_repeaters, function (key, repeater) {
        ...
        jQuery.each(repeater, function (key, value) {
            ...
            if (repeater['controllers']['data'].attr('data-required')) {
                repeater['controllers']['start'].addClass('gfield_contains_required');
            }

            jQuery.each(repeater['children'], function (key, value) {
                gfRepeater_setRepeaterChildAttrs(formId, repeaterId, jQuery(repeater['children'][key]['element']), 1);
                if (this.conditionalLogic) {
                    gfRepeater_conditionalLogic_set(formId, repeaterId, key, 1);
                }
            });
